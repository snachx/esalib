...
public class ESAIndexSearcher {
    ...
    public static void main(String[] args) throws ClassNotFoundException, SQLException, IOException {
        String stopWords = "res/stopwords.en.txt";
        String stemmerClass = "org.tartarus.snowball.ext.EnglishStemmer";
        String lang = "en";


        DB db = new DB("mysql://root:123456@localhost/wiki");
        DB esaDb = new DB("mysql://root:123456@localhost/wiki");

        ESAAnalyzer searcher = new ESAAnalyzer(esaDb, lang, stopWords, stemmerClass);
        IConceptVector cv = searcher.getConceptVector(Utils.readBinaryStream(new FileInputStream("linux.txt")));
        cv = ESAAnalyzer.trim(cv, 100);


        ESAIndexSearcher eis = new ESAIndexSearcher(db, "/tmp/esa_vector_index");
    ...
    public IConceptVector cSimpleSearch(byte[] v, int limit) throws IOException, SQLException {
        ...
        String query = "SELECT esa_search(?, '"
                + indexPath + "', " + limit + ") AS res_vector";
        PreparedStatement psClEsa = indexDb.getConnection().prepareStatement(query);
        psClEsa.setBytes(1, v);
        ResultSet clres;
        try {
